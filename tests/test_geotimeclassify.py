...
import timeit
import time
from typing import Callable, Generator, Sequence, Iterator, Tuple, List, Union
...
TestGen = Iterator[Union[Tuple[str, (Callable[[], None])], Callable[[], None]]]



def main():
    #functions that generate suites of tests    
    test_generators: List[Callable[[], TestGen]] = [
        time_resolution_algorithm,
        time_resolution_whole_pipeline,
        #### insert more test generators here ####
    ]

    total_passed = 0
    total_failed = 0
    for test_generator in test_generators:
        print(f'---------------------{test_generator.__name__}---------------------')
        passed = 0
        failed = 0
        for test in test_generator():
            ...
            try:
                t = timeit.Timer(test)
                print(f'PASSED {name}. runtime: {t.timeit(1):.04f} (seconds)')
                passed += 1
            except Exception as e:
                print(f'FAILED {name}: {e}')
                failed += 1
        
        print(f'Completed tests for `{test_generator.__name__}`')
    ...
    print(f'PASSED {total_passed} tests, FAILED {total_failed} tests')
...
def time_resolution_algorithm() -> TestGen:
...
def time_resolution_whole_pipeline() -> TestGen:
